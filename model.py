...
class DCGAN(object):
  ...
  def build_model(self):
    ...
    self.d_loss_real = tf.reduce_mean(
      input_tensor=sigmoid_cross_entropy_with_logits(self.D_logits, tf.ones_like(self.D)))
    self.d_loss_fake = tf.reduce_mean(
      input_tensor=sigmoid_cross_entropy_with_logits(self.D_logits_, tf.zeros_like(self.D_)))
    self.g_loss = tf.reduce_mean(
      input_tensor=sigmoid_cross_entropy_with_logits(self.D_logits_, tf.ones_like(self.D_)))
  ...
  def save(self, checkpoint_dir, step, filename='model', ckpt=True, frozen=False):
    ...
    if frozen:
      tf.io.write_graph(
              tf.compat.v1.graph_util.convert_variables_to_constants(self.sess, self.sess.graph_def, ["generator_1/Tanh"]),
              checkpoint_dir,

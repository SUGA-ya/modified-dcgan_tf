...
#get_stddev = lambda x, k_h, k_w: 1/math.sqrt(tf.shape(k_w*k_h*x)[-1])
...
def visualize(sess, dcgan, config, option, sample_dir='samples'):
  image_frame_dim = int(math.ceil(config.batch_size**.5))#即开根号
  if option == 0:
    ...
    for idx in xrange(dcgan.generate_test_images):
      print(" [*] %d" % idx)
      ...
      print(os.path.join(sample_dir, 'test_arange_%s.png' % (idx)))

  elif option == 2:
    values = np.arange(0, 1, 1./config.batch_size)
    for idx in [random.randint(0, dcgan.generate_test_images - 1) for _ in xrange(dcgan.generate_test_images)]:
      print(" [*] %d" % idx)
